...
import com.github.sh0nk.matplotlib4j.axes.kwargs.Line3DBuilderImpl;
...
public class PlotBuilderImpl implements PlotBuilder {
    ...
    private final CompositeAxesBuilder<PlotBuilder> innerBuilder = new CompositeAxesBuilder<>(this);

    // Variable defining the builder for all the **kwargs
    private final Line3DBuilder<PlotBuilder> kwargsBuilder = new Line3DBuilderImpl<>(innerBuilder);
    ...
    @Override
    public PlotBuilder add(List<? extends Number> x, List<? extends Number> y, List<? extends Number> z, String fmt) {
        innerBuilder.addToArgs(x);
        innerBuilder.addToArgs(y);
